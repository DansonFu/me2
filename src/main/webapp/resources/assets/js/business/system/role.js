jQuery(function($) {
	...
	jQuery(grid_selector).jqGrid({
		url:basePath+"admin/role/getList",//获取数据的地址
		mtype:"POST",
		datatype: "json",//从服务器端返回的数据类型
		height: "auto",//表格高度
		rownumbers:true,//显示行号
		colNames:['ID', '角色名', '说明', '创建时间', '修改时间'/*,'操作'*/],//列显示名称
		colModel:[
			...
			{name:'detail',index:'detail', width:'30%', editable:true, edittype:'text', searchoptions:{sopt:['eq','ne','cn','nc']},search:false},
			...
			{name:'updateTime',hidden:true,index:'updateTime', width:'16%', editable:false ,searchoptions:{
				dataInit : function (elem){
					$(elem).datetimepicker({timeFormat: "HH:mm:ss",dateFormat: "yy-mm-dd" , autoclose:true});
				},
				sopt:['bw','ew','bn','en']
	...
	jQuery(grid_selector).jqGrid('navGrid',pager_selector,
		{ 	//navbar options
			edit: checkedButton('roleEdit'),
			editicon : 'icon-pencil blue',
			add: checkedButton('roleAdd'),
			addicon : 'icon-plus-sign purple',
			del: checkedButton('roleDel'),
			delicon : 'icon-trash red',
			search: checkedButton('roleSearch'),

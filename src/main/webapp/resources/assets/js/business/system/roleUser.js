...
jQuery(function($) {
	...
	jQuery(grid_selector).jqGrid({
		url:basePath+"admin/roleUser/getUserList",//获取数据的地址
		mtype:"POST",
		datatype: "json",//从服务器端返回的数据类型
		height: 394,//表格高度
		rownumbers:true,//显示行号
		colNames:['ID', '账号', '姓名'],//列显示名称
		colModel:[
			{name:'userId',index:'userId', width:'5%', editable:false,hidden:true,key:true},
			{name:'account',index:'account', width:'30%', editable:true, edittype:'text', searchoptions:{sopt:['eq','ne','cn','nc']},editrules:{required:true}},
			{name:'name',index:'name', width:'30%', editable:true, edittype:'text', searchoptions:{sopt:['eq','ne','cn','nc']}},
		],//列属性
		...
		loadComplete : function() {
			...
			if(selected!=""){
				$.ajax({
					async:true,
					cache:false,
					url:basePath+"/admin/roleUser/getUserSelected",
					type:"post",
					datatype:"json",
					...
					success:function(data){
						...
						$.each(rows, function (index, roleUser) {
							jQuery("#grid-table").jqGrid('setSelection', roleUser.userId);
						});
					}
				});
...
//				console.log(selectedIds.join(","));
				$.ajax({
					async:true,
					cache:false,
					url:basePath+"admin/roleUser/saveRoleSelected",
					type:"post",
					datatype:"json",
...
jQuery(function($) {
	...
	jQuery(grid_selector).jqGrid({
		url: basePath+"admin/roleUser/getRoleList",
		mtype:"POST",
		datatype: "json",
		rownumbers:true,
		height: "auto",
		colNames:[ 'ID', '角色名', '说明'],
		colModel:[
			{name:'roleId', key:true, index:'roleId', width:80, editable: false, hidden:true,search:false, viewable:true},
			{name:'name',index:'name', width:80, editable: true,search:true,stype:'text',searchoptions:{sopt:['cn']}},
			{name:'detail',index:'detail', width:50, editable: true,search:true,stype:'text',searchoptions:{sopt:['cn']}}
		], 

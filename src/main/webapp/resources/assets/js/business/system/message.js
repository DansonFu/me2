jQuery(function($) {
	...
	jQuery(grid_selector).jqGrid({
		url:basePath+"admin/message/getList",//获取数据的地址
		mtype:"POST",
		datatype: "json",//从服务器端返回的数据类型
		height: "auto",//表格高度
		rownumbers:true,//显示行号
		colNames:['ID', '标题', '内容', '状态', '类型', '创建人','创建时间','修改时间'/*,'操作'*/],//列显示名称
		colModel:[
			...
			{name:'status',index:'status', width:'6%', editable:true, edittype:'select',editoptions:{value:"0:已发布;1:未发布;2:禁用"},stype:'select', searchoptions:{sopt:['eq','ne'],value:"0:已发布;1:未发布;2:禁用"}, formatter:'select',search:false},
			...
			{name:'createUser.name', width:'8%', editable:false,search:false},
			...
			{name:'myac',index:'', width:80, fixed:true, sortable:false, resize:false,search:false,viewable:false,
				...
				formatoptions:{ 
					...
					editformbutton:true, 
					editOptions:{closeAfterEdit: true, recreateForm: true, beforeShowForm:beforeEditCallback, afterSubmit : onEditafterSubmit,beforeSubmit:onEditBeforeSubmit}
				}
			}*/
		],//列属性
		...
		caption: "通知公告管理列表",//表格名称
		autowidth: true,//首次被创建时会根据父元素比例重新调整表格宽度
		editurl:basePath+"admin/message/oper"//对表格增删改操作提交的url
	});
	...
	jQuery(grid_selector).jqGrid('navGrid',pager_selector,
		{ 	//navbar options
			edit: checkedButton('messageEdit'),
			editicon : 'icon-pencil blue',
			add: checkedButton('messageAdd'),
			addicon : 'icon-plus-sign purple',
			del: checkedButton('messageDel'),
			delicon : 'icon-trash red',
			search: checkedButton('messageSearch'),

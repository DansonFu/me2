jQuery(function($) {
	...
	jQuery(grid_selector).jqGrid({
		url:basePath+"erp/payReceiver/getReceiverList",//获取数据的地址
		mtype:"POST",
		datatype: "json",//从服务器端返回的数据类型
		height: "auto",//表格高度
		rownumbers:true,//显示行号
		colNames:['编号', '订单编号','内容', '供应商姓名','供应商电话', '应收款金额（单位：元）','已收款金额（单位：元）','未收款金额（单位：元）','本次收款金额（单位：元）','状态','类型','创建时间','订单号', '客户编号', '操作'],//列显示名称
		colModel:[
			...
			{name:'type', width:'8%', formoptions: {rowpos: 5,colpos: 1},editable:true, edittype:'select',editoptions:{value:"1:销售收款;4:采购退货收款"},stype:'select', searchoptions:{sopt:['eq','ne'],value:"1:销售收款;4:采购退货收款"}, formatter:'select'},
			{name:'creatTime',index:'creatTime', width:'10%', editable:false, hidden:false,editoptions:{readonly:true},searchoptions:{
	...
	jQuery("#grid-table").jqGrid('navGrid',"#grid-pager").jqGrid('navButtonAdd',"#grid-pager",{
		...
		onClickButton:function(){
			...
			window.location = basePath + "erp/payReceiver/getReceiverListExport?postData="+postData;
		}, 
		position: "last", 
		title:"导出excel", 
		cursor: "pointer",

jQuery(function($) {
	...
	jQuery(grid_selector).jqGrid({
		url:basePath+"erp/buyOrder/getList",//获取数据的地址
		mtype:"POST",
		...
		colModel:[
			...
			{name:'orderNumber', width:'8%', editable:false,searchoptions:{sopt:['eq','ne','cn','nc']}},
			...
			{name:'productType', width:'8%', editable:true,edittype:'select',editoptions:{value:"1:外购需检;2:外购免检"},stype:'select',searchoptions:{sopt:['eq','ne'],value:"1:外购需检;2:外购免检"},formatter:'select'},
...
//	});
	...
	jQuery("#grid-table").jqGrid('navGrid',"#grid-pager").jqGrid('navButtonAdd',"#grid-pager",{
		...
		onClickButton:function(){
			...
			window.location=basePath+"erp/toEditBuyOrder?orderId="+myid;
		}, 
		position: "first", 
		title:"修改订单", 
		cursor: "pointer",
	...
	jQuery("#grid-table").jqGrid('navGrid',"#grid-pager").jqGrid('navButtonAdd',"#grid-pager",{
		...
		onClickButton:function(){
			window.location=basePath+"erp/toAddBuyOrder";
		}, 
		position: "first", 
		title:"新增订单", 
		cursor: "pointer",
	...
	jQuery("#grid-table").jqGrid('navGrid',"#grid-pager").jqGrid('navButtonAdd',"#grid-pager",{
		...
		onClickButton:function(){
			...
			window.location = basePath + "erp/buyOrder/getListExport?postData="+postData;
		}, 
		position: "last", 
		title:"导出excel", 
		cursor: "pointer",
...
function orderProcessSubmit(status, orderId){
	...
	$.ajax({
		async:false,
		url:basePath+"erp/buyOrder/getOrderMaterialList",
		type:"post",
		datatype:"json",
	...
	if(!hadRecords){
		fDialogAlert("请为订单添加商品");
		return;
	}
	//询问框是否提交
	$.fDialog.alert("您确定要提交该采购订单？","question",{
		drag:true,
		title : "系统提示信息",
		buttons : [{text:'提交',value:'yes'},{text:'取消',cls:'gray',value:'no'}],
		buttonClick : function(v){
			if(v=='yes'){
				$.ajax({
					url:basePath+"erp/buyOrder/startProcess",
					type:"post",
					datatype:"json",
					data:{"id":orderId},
					success:function(data){
...
function viewOrderProcessAudit(status, orderId){
	if(status==1){
		fDialogAlert("该采购订单未提交！");
		return;
	}
	window.location=basePath+"/erp/buyOrder/buyOrderAuditHis?id="+orderId;
}
jQuery(function($) {
	...
	jQuery(grid_selector).jqGrid({
		url:basePath+"erp/buyReturnOrder/getList",//获取数据的地址
		mtype:"POST",
		datatype: "json",//从服务器端返回的数据类型
		height: "auto",//表格高度
		rownumbers:true,//显示行号
		colNames:['订单号', '订单编号','供应商','供应商', '地址', '电话', '手机', '总金额','创建时间','修改时间','类型','创建人','状态','付款方式','供应类别','日期','备注','回馈','操作'],//列显示名称
		colModel:[
			{name:'orderId',index:'orderId', width:'5%', editable:false,hidden:true,key:true},
			{name:'orderNumber',width:'10%', editable:false,searchoptions:{sopt:['eq','ne','cn','nc']}},
			...
			{name:'createTime',width:'10%', editable:false, hidden:false, searchoptions:{
			...
			{name:'type',index:'type', width:'8%', editable:true,hidden:true,searchoptions:{sopt:['eq','ne']}},
...
//	});
	...
	jQuery("#grid-table").jqGrid('navGrid',"#grid-pager").jqGrid('navButtonAdd',"#grid-pager",{
		...
		onClickButton:function(){
			...
			window.location=basePath+"erp/toEditBuyReturnOrder?orderId="+myid;
		}, 
		position: "first", 
		title:"修改订单", 
		cursor: "pointer",
	...
	jQuery("#grid-table").jqGrid('navGrid',"#grid-pager").jqGrid('navButtonAdd',"#grid-pager",{
		...
		onClickButton:function(){
			window.location=basePath+"erp/toAddBuyReturnOrder";
		}, 
		position: "first", 
		title:"新增订单", 
		cursor: "pointer",
	...
	jQuery("#grid-table").jqGrid('navGrid',"#grid-pager").jqGrid('navButtonAdd',"#grid-pager",{
		...
		onClickButton:function(){
			...
			window.location = basePath + "erp/buyReturnOrder/getListExport?postData="+postData;
		}, 
		position: "last", 
		title:"导出excel", 
		cursor: "pointer",
...
//					return;
//				}
//				window.location=basePath+"/erp/sellOrder/sellOrderAuditHis?id="+rowid;
//			}else{
//				alert("请选择要进行查看审核记录的订单信息");
//			}
//			
//		}, 
//		position: "last", 
...
function orderProcessSubmit(status, orderId){
	...
	if(status!=1){
		fDialogAlert("该采购退货订单记录申请已提交或完成！");
		return;
	}
	//询问框是否提交
	$.fDialog.alert("您确定要为该采购退货订单提交申请？","question",{
		drag:true,
		title : "系统提示信息",
		buttons : [{text:'提交',value:'yes'},{text:'取消',cls:'gray',value:'no'}],
		buttonClick : function(v){
			if(v=='yes'){
				$.ajax({
					url:basePath+"erp/buyReturnOrder/startProcess",
					type:"post",
					datatype:"json",
					data:{"id":orderId},
					success:function(data){
...
function viewOrderProcessAudit(status, orderId){
	...
	window.location=basePath+"/erp/buyReturnOrder/buyReturnOrderAuditHis?id="+orderId;
}
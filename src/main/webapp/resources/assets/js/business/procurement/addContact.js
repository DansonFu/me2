...
function billSubmit(isStartProcess){
	...
	for(var i=0;i<griddata.length;i++){
		...
		griddata[i]['shipmentEnterprise']['seId']=griddata[i]['seId'];
	}
	...
	var jsondata = {"name":name,"address":address,"phone":phone,"mobile":mobile,"email":email,"zipcode":zipcode,"contactPerson":contactPerson,"qq":qq,"fax":fax,"taxNumber":taxNumber,"openBank":openBank,"account":account,"invoiceAddress":invoiceAddress,"note":note,"griddata":griddata};

    $.ajax({
		url:basePath+"erp/supplierInfo/save",
		type:"post",
		datatype:"json",
		data: jsondata,
		success:function(data){
			if(data.message=="success"){
				window.location=basePath+"erp/toSupplierInfo";
			}else{
				fDialogAlert("保存失败");
			}
		}
...
function addShipmentEnterpriseSubmit(){
	...
	var griddata=jQuery("#grid-table").jqGrid("getRowData");
	for(var i=0;i<griddata.length;i++){
		if(subIframeSelectDataShipmentEnterprise.seId==griddata[i].seId){
			fDialogAlert("不能添加相同的物流商");
			return;
		}
	}
	jQuery("#grid-table").jqGrid().addRowData(subIframeSelectDataShipmentEnterprise.seId,subIframeSelectDataShipmentEnterprise,"last");
	$("#shipmentEnterpriseId").val("");
	$("#shipmentEnterpriseName").val("");
	subIframeSelectDataShipmentEnterprise=null;
}

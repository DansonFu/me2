...
jQuery(function($) {
	...
	jQuery(grid_selector).jqGrid({
		...
		colNames:['ID', '产品','产品名称','产品编号', '规格','色号','单价','件数', '总片数', '面积','金额','优惠金额'],//列显示名称
		colModel:[
			{name:'orderMaterialId',index:'orderMaterialId', width:'5%', editable:false,hidden:true},
			{name:'productId',index:'productId', width:'5%', editable:true, edittype:'select',editoptions: {dataUrl: basePath+'erp/sellOrder/getSelect'},searchoptions:{sopt:['eq','ne']},hidden:true},
			{name:'name',index:'name', width:'10%', editable:false,searchoptions:{sopt:['eq','ne']}},
			{name:'num',index:'num', width:'10%', editable:true,searchoptions:{sopt:['eq','ne']}},
			{name:'spec',index:'spec', width:'10%', editable:false,searchoptions:{sopt:['eq','ne']}},
			{name:'color',index:'color', width:'10%', editable:false,edittype:'select',editoptions:{value:"0:无;1:1;2:2;3:3"},stype:'select',searchoptions:{sopt:['eq','ne'],value:"0:无;1:1;2:2;3:3"}, formatter:'select'},
			{name:'price',index:'price', width:'10%', editable:false,searchoptions:{sopt:['eq','ne']}},
			...
			{name:'discount',index:'discount', width:'10%', editable:false,searchoptions:{sopt:['eq','ne']}}
		],//列属性
...
function billSubmit(isStartProcess){
	...
	for(var i=0;i<griddata.length;i++){
		griddata[i]['product']={};
		griddata[i]['product']['productId']=griddata[i]['productId'];
	}
	if(isStartProcess==true &&  griddata.length==0){
	...
	var jsondata =jsondata = {"orderPreType":orderPreType,"isStartProcess":isStartProcess,"phone":phone,"note":note,"address":address,"mobile":mobile,"needPay":needPay,"productType":productType,"amount":amount,"griddata":griddata,"name":contactName,"contact.contactId":contactId,"contact.name":contactNameHidden};

    $.ajax({
		url:basePath+"erp/buyOrder/saveOrder",
		type:"post",
		datatype:"json",

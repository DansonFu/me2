...
jQuery(function($) {
	...
	jQuery(grid_selector).jqGrid({
		...
		colModel:[
			...
			{name:'price',index:'price', width:'10%', editable:false},
...
function billSubmit(isStartProcess){
	...
	var jsondata =jsondata = {"orderPreType":orderPreType,"isStartProcess":isStartProcess,"phone":phone,"note":note,"address":address,"mobile":mobile,"needPay":needPay,"productType":productType,"amount":amount,"griddata":griddata,"name":contactName,"contact.contactId":contactId,"contact.name":contactNameHidden,"shipmentEnterprise.seId":seId};

    $.ajax({
		url:basePath+"erp/sellOrder/saveOrder",
		type:"post",
		datatype:"json",
...
function billReset(){
	...
	$("#contactId").val("");
	$("#contactNameHidden").val("");
	$("#shipmentEnterpriseId").val("");
	$("#shipmentEnterpriseId").html(null);
...
function addMaterialSubmit(){
	...
	if(addMaterialType=="edit"){
		...
		//$("#grid-table").footerData("set",{area:"合计",amount:sum});

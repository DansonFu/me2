jQuery(function($) {
	...
	jQuery(grid_selector).jqGrid({
		url:basePath+"erp/logistics/getSelectOrderList?",//获取数据的地址
		mtype:"POST",
		datatype: "json",//从服务器端返回的数据类型
		height: "auto",//表格高度
		rownumbers:true,//显示行号
		colNames:['订单号', '订单编号','客户','客户', '地址', '电话', '手机', '总金额','类型','创建人','状态','付款方式','供应类别','日期','备注','回馈','创建时间','修改时间','物流商名称','物流商id'],//列显示名称
		colModel:[
			...
			{name:'amount',width:100, editable:true,hidden:true,searchoptions:{sopt:['eq','ne','lt','le','gt','ge']}},
			{name:'type',width:100, editable:true,hidden:true,searchoptions:{sopt:['eq','ne','cn','nc']}},
			...
			{name:'shipmentEnterprise.name',width:60,hidden:true},
			{name:'shipmentEnterprise.seId',width:60,hidden:true}
		],//列属性
		...
		beforeSelectRow: function(rowid, e){	//设置单选
			...
			parent.subIframeSetSelectDataLogisticsOrder(rowData);
		    return (true);
		},
		gridComplete: function(){	//隐藏表头复选框 id为：cb+jqgrid的id
			$("#cb_grid-table").hide();
			//回显设置
			if(requestLogisticsOrderId){
				jQuery(grid_selector).jqGrid('setSelection', requestLogisticsOrderId);
			}
		},

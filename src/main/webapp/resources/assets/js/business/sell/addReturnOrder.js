...
jQuery(function($) {
	...
	jQuery(grid_selector).jqGrid({
		...
		colNames:['ID', '产品','产品名称', '规格','产品编号','色号','单价','件数', '总片数', '面积','金额','优惠金额','单片面积','每箱片数'],//列显示名称
		colModel:[
			...
			{name:'product.num',index:'num', width:'10%'},
			...
			{name:'product.area',index:'product.area', width:'10%', editable:false,hidden:true},
			{name:'product.silces',index:'product.silces', width:'10%', editable:false,hidden:true}
		],//列属性
...
//		},
		gridComplete:function(){
			...
			$(grid_selector).footerData("set",{sum:"合计",discount:sum});
		},
...
function billSubmit(isStartProcess){
	...
	var jsondata =jsondata = {"orderPreType":orderPreType,"isStartProcess":isStartProcess,"orderId":orderId,"amount":amount,"griddata":griddata,"note":note};
...
function addMaterialSubmit(){
	...
	$.ajax({
		...
		success:function(data){
			if(data.orderMaterial.pieces<a||data.orderMaterial.slices<b){
				isPass=2;
			}
		}
    });
	...
	$("#grid-table").footerData("set",{sum:"合计",amount:discount});
}
...
function selectOrderOnclick(){
	$.fDialog({
		title : '<b>销售订单列表</b>',
		content : basePath+'erp/selectReturnOrder?type=1',
		module : 'iframe',
		w : '80%',
		h : '522',
		...
		beforeRemove : function($f){
			if(subIframeSelectDataSellOrder){
				...
				$("#shipmentEnterprisetName").val(subIframeSelectDataSellOrder["shipmentEnterprise.name"]);

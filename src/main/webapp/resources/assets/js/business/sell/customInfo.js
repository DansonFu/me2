...
jQuery(function($) {
	...
	jQuery(grid_selector).jqGrid({
		url:basePath+"erp/customInfo/getList",//获取数据的地址
		mtype:"POST",
		...
		colModel:[
			...
			{name:'address',index:'address', width:'20%', editable:true,formoptions: {colpos: 1, rowpos: 2}, edittype:'text', searchoptions:{sopt:['eq','ne','cn','nc']},editrules:{required:true}},
			...
			{name:'zipcode',index:'zipcode', search:false,width:'8%', formoptions: {colpos: 1, rowpos: 6},editable:true,searchoptions:{sopt:['eq','ne','cn','nc']}},
			{name:'contactPerson',index:'contactPerson',search:false, width:'8%', formoptions: {colpos: 1, rowpos: 7},hidden:true,editable:true, editrules:{edithidden:true},searchoptions:{sopt:['eq','ne','cn','nc'],searchhidden:true}},
			...
			{name:'taxNumber',index:'taxNumber',search:false, width:'8%',formoptions: {colpos: 2, rowpos: 3}, hidden:true,editable:true, editrules:{edithidden:true},searchoptions:{sopt:['eq','ne','cn','nc'],searchhidden:true}},
			{name:'openBank',index:'openBank', search:false,width:'8%', formoptions: {colpos: 2, rowpos: 4},hidden:true,editable:true, editrules:{edithidden:true},searchoptions:{sopt:['eq','ne','cn','nc'],searchhidden:true}},
			{name:'account',index:'account',search:false, width:'8%', formoptions: {colpos: 2, rowpos: 5},hidden:true,editable:true, editrules:{edithidden:true},searchoptions:{sopt:['eq','ne','cn','nc'],searchhidden:true}},
			...
			{name:'createTime',index:'createTime', width:'16%', search:false,editable:false,hidden:true, searchoptions:{
			...
			{name:'updateTime',index:'updateTime', width:'16%', search:false,editable:false, hidden:true,searchoptions:{
				dataInit : function (elem){
					$(elem).datetimepicker({timeFormat: "HH:mm:ss",dateFormat: "yy-mm-dd" , autoclose:true});
				},
				sopt:['bw','ew','bn','en'],
		...
		caption: "客户信息管理列表",//表格名称
		autowidth: true,//首次被创建时会根据父元素比例重新调整表格宽度
		editurl:basePath+"erp/customInfo/oper",//对表格增删改操作提交的url,
		//设置jqgrid复选框为单选形式
	...
	jQuery(grid_selector).jqGrid('navGrid',pager_selector,
		{ 	//navbar options
			...
			search: checkedButton('customSearch'),
	...
	$(function(){
						...
						buttons: [
							{
								...
								click: function() {
									...
									$('#importExcelForm').ajaxSubmit({
										url: basePath+"erp/customInfo/importExcel",
										type:"post",
										dataType:'json',
										iframe: true,
		...
		jQuery("#grid-table").jqGrid('navGrid',"#grid-pager").jqGrid('navButtonAdd',"#grid-pager",{
			...
			onClickButton:function(){
				...
				window.location = basePath + "erp/customInfo/getListAllExport?postData="+postData;
			}, 
			position: "last", 
			title:"导出excel", 
			cursor: "pointer",
		...
		if(checkedButton('customDel')){
			jQuery("#grid-table").jqGrid('navGrid',"#grid-pager").jqGrid('navButtonAdd',"#grid-pager",{
				...
				onClickButton:function(){
					...
					$.ajax({
						url:basePath+"erp/customInfo/del",
						type:"post",
						datatype:"json",
						data: {"contactId":myid},
						success:function(data){
		...
		if(checkedButton('customEdit')){
			jQuery("#grid-table").jqGrid('navGrid',"#grid-pager").jqGrid('navButtonAdd',"#grid-pager",{
				...
				onClickButton:function(){
					...
					window.location=basePath+"erp/customInfo/toEditContact?contactId="+myid;
				}, 
				position: "first", 
				title:"修改客户", 
				cursor: "pointer",
		...
		if(checkedButton('customAdd')){
			jQuery("#grid-table").jqGrid('navGrid',"#grid-pager").jqGrid('navButtonAdd',"#grid-pager",{
				caption:"", 
				buttonicon:"icon-plus-sign green", 
				onClickButton:function(){
					window.location=basePath+"erp/customInfo/toAddContact";
				}, 
				position: "first", 
				title:"添加客户", 
				cursor: "pointer",

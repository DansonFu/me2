jQuery(function($) {
	...
	jQuery(grid_selector).jqGrid({
		url:basePath+"erp/sellOrder/getSelectOrderList?type="+requestType,//获取数据的地址
		mtype:"POST",
		datatype: "json",//从服务器端返回的数据类型
		height: "auto",//表格高度
		rownumbers:true,//显示行号
		colNames:['订单号', '订单编号','客户','客户', '地址', '电话', '手机', '总金额','类型','创建人','状态','付款方式','供应类别','日期','备注','回馈','创建时间','修改时间'],//列显示名称
		colModel:[
			{name:'orderId',width:50, editable:false,hidden:true,key:true},
			{name:'orderNumber', width:50, editable:false,hidden:false,searchoptions:{sopt:['eq','ne','cn','nc']}},
